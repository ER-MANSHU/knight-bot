...
from Music import client as KNIGHT

# To Block a PM'ed User
@KNIGHT.on_message(filters.private & filters.command("block", [".", "/"]) & filters.me & ~filters.edited)
async def ubblock(_, message: Message):
  shit_id = message.chat.id
  gonna_block_u = await message.edit_text("`Blocking User...`")
  try:
    await KNIGHT.block_user(shit_id)
...
@KNIGHT.on_message(filters.command("unblock", [".", "/"]) & filters.me & ~filters.edited)
async def ubblock(_, message: Message):
  good_bro = int(message.command[1])
  gonna_unblock_u = await message.edit_text("`Unblocking User...`")
  try:
    await KNIGHT.unblock_user(good_bro)
...
@KNIGHT.on_message(filters.private & filters.command("chats", [".", "/"]) & filters.me & ~filters.edited)
...
# Leave From a Chat
@KNIGHT.on_message(filters.command("kickme", [".", "/"]) & filters.me & ~filters.edited)
async def ubkickme(_, message: Message):
  i_go_away = await message.edit_text("`Leaving This Chat...`")
  try:
    await KNIGHT.leave_chat(message.chat.id)
...
# Alive Message
@KNIGHT.on_message(filters.command("alive", [".", "/"]) & filters.me & ~filters.edited)
