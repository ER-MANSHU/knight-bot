...
@KNIGHTUB.on_message(filters.private & filters.command("block", [".", "/"]) & filters.me & ~filters.edited)
async def ubblock(_, message: Message):
  shit_id = message.chat.id
  gonna_block_u = await message.edit_text("`Blocking User...`")
  try:
    await KNIGHTUB.block_user(shit_id)
...
@KNIGHTUB.on_message(filters.command("unblock", [".", "/"]) & filters.me & ~filters.edited)
async def ubblock(_, message: Message):
  good_bro = int(message.command[1])
  gonna_unblock_u = await message.edit_text("`Unblocking User...`")
  try:
    await KNIGHTUB.unblock_user(good_bro)
...
@KNIGHTUB.on_message(filters.private & filters.command("chats", [".", "/"]) & filters.me & ~filters.edited)
...
# Leave From a Chat
@KNIGHTUB.on_message(filters.command("kickme", [".", "/"]) & filters.me & ~filters.edited)
...
async def ubalive(_, message: Message):
  ...
  await message.reply_photo(alive_pic, caption=f"**☑️ KNIGHT MUSIC BOT is Alive 🌀** \n\n**🤖 Version** \n ↳**Bot Version:** `2.5` \n ↳**Userbot Version:** `0.0` \n\n**🐬 Info**\n ↳**Music Bot:** @{BOT_USERNAME} \n ↳**Owner:** [Click Here](tg://user?id=1669178360)")
